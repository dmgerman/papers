...
** Who wrote this code? What changes lead to this function's current implementation?

These are typical questions that developers (and sometimes layers) ask during their work.
...
In general, one would expect to always ask blame to ignore whitespace. However, sometimes this is not possible (such as the
...
split are not addressed properly (the ignore-whitespace option does not ignore them). Second, lines that were mostly
authored by Developer A are now assigned to Developer B because she was the last one to modify them.
...
file in our example would look like this. Note how it correctly shows that the only changes made by C to the source code
were the replacement of int with long three places.
...
history has been recorded using git. The details of cregit's implementation can be found in this Working Paper (currently under review).
...
The source code is then colours according to the person who last added the token. The right hand side of the view shows an overview of the
...
The views are highly interactive. For example, one can select to highlight a commit (top middle XXX box). In this case, all the code is
grayed, except for the tokens that were added in by commit, as shown below.
...
These features can be easily combined. You can select the age of the code by a specific author.
